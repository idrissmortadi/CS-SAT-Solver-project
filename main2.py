    ...
    def add_obstacle_constraints(self, obstacles: Set[Position], robots: List[Robot]):
        ...
        for obstacle in obstacles:
            if not (0 <= obstacle.x < self.width and 0 <= obstacle.y < self.height):
                raise ValueError(f"Obstacle at position ({obstacle.x}, {obstacle.y}) is outside the warehouse bounds")
                ...
                if t < self.time_horizon:
                    for x in range(self.width):
                        for y in range(self.height):
                            ...
                            next_positions = []
                            for dx, dy in self.moves:  # includes (0,0) for staying in place
...
def solve_warehouse_problem(
    ...
    for robot in robots:
        if not (0 <= robot.start.x < width and 0 <= robot.start.y < height):
            raise ValueError(f"Robot {robot.id} start position is outside the warehouse bounds")
        if not (0 <= robot.goal.x < width and 0 <= robot.goal.y < height):
            raise ValueError(f"Robot {robot.id} goal position is outside the warehouse bounds")
        if Position(robot.start.x, robot.start.y) in obstacles:
            raise ValueError(f"Robot {robot.id} start position overlaps with an obstacle")
        if Position(robot.goal.x, robot.goal.y) in obstacles:
            raise ValueError(f"Robot {robot.id} goal position overlaps with an obstacle")
            ...
            if paths:
                ...
                for robot_id, path in sorted(paths.items()):
                    ...
                    for t, pos in enumerate(path):
                        move_type = "stay" if (t > 0 and path[t] == path[t-1]) else "move"
